...
define(["jquery", "underscore", "webuploader", "jquery.jplayer", "bootstrap"], function (a, b, c) {
	var d = {
		defaultoptions: {
			...
			var b = this;b.modalobj.find("#li_network").removeClass("hide"), b.modalobj.find(".modal-body").append(b.buildHtml().remoteDialog), b.modalobj.find(".btn-primary").click(function () {
				var c = b.modalobj.find("#networkurl").val();c.length > 0 && a.getJSON("./index.php?c=utility&a=file&do=fetch", { url: c }, function (a) {
					a.message && alert(a.message), a && (b.finish([a]), a = {});
				});
			});
		}, initLocal: function initLocal() {
			var a = this;a.modalobj.find("#li_history_image").removeClass("hide"), a.modalobj.find("#history_image")[0] || a.modalobj.find(".modal-body").append(this.buildHtml().localDialog);a.modalobj.find("#history_image");a.localPage(1);
		}, localPage: function localPage(c) {}, selectImage: function selectImage(b) {}, initLocalAudio: function initLocalAudio() {
			var a = this;a.modalobj.find("#li_history_audio").removeClass("hide"), a.modalobj.find("#history_audio")[0] || a.modalobj.find(".modal-body").append(this.buildHtml().localAudioDialog);a.modalobj.find("#li_history_audio");a.localAudioPage(1);
		}, localAudioPage: function localAudioPage(c) {
			var d = this,
			    e = d.modalobj.find("#history_audio");return a.getJSON("./index.php?c=utility&a=file&do=local&type=audio&pagesize=5", { page: c }, function (c) {
				...
				var b = [];e.find(".history-content .btn-primary").each(function () {
					b.push(d.modalobj.find("#history_audio").data("attachment")[a(this).attr("attachid")]), a(this).removeClass("btn-primary");
				}), d.finish(b);
			}), !1;
		}, playAudio: function playAudio() {
			var b = this,
			    c = b.modalobj.find("#history_audio");a(".audio-player-play").click(function () {
				var b = a(this).attr("attach");if (b) {
					if (a("#player")[0]) var d = a("#player");else {
						var d = a('<div id="player"></div>');a(document.body).append(d);
					}d.data("control", a(this)), d.jPlayer({ playing: function playing() {
						...
						}, swfPath: "resource/components/jplayer", supplied: "mp3,wma,wav,amr", solution: "html, flash" }), d.jPlayer("setMedia", { mp3: a(this).attr("attach") }).jPlayer("play"), a(this).find("p").hasClass("fa-stop") ? d.jPlayer("stop") : (c.find(".fa-stop").removeClass("fa-stop").addClass("fa-play"), d.jPlayer("setMedia", { mp3: a(this).attr("attach") }).jPlayer("play"));
				}
			});
			...
			function b(b) {
				var c = a('<li id="' + b.id + '"><p class="title">' + b.name + '</p><p class="imgWrap"></p></li>'),
				    ...
				    h = a('<p class="error"></p>'),
				    j = function j(a) {
					switch (a) {case "exceed_size":
							text = "文件大小超出";break;case "interrupt":
							text = "上传暂停";break;default:
							text = "上传失败，请重试";}h.text(text).appendTo(c);
				};"invalid" === b.getStatus() ? j(b.statusText) : (g.text("预览中"), d.makeThumb(b, function (b, c) {
					if (b) return void g.text("不能预览");var d = a('<img src="' + c + '">');g.empty().append(d);
					...
					var c,
					    e = a(this).index();switch (e) {case 0:
							return void d.removeFile(b);case 1:
							b.rotation += 90;break;case 2:
							b.rotation -= 90;}supportTransition ? (c = "rotate(" + b.rotation + "deg)", g.css({ "-webkit-transform": c, "-mos-transform": c, "-o-transform": c, transform: c })) : g.css("filter", "progid:DXImageTransform.Microsoft.BasicImage(rotation=" + ~ ~(b.rotation / 90 % 4 + 4) % 4 + ")");
				}), i.options.multiple && k.find(".fileinput-button").show(), c.insertBefore(k.find(".fileinput-button"));
			}function e(b) {
				var c = a("#" + b.id);delete percentages[b.id], f(), c.off().find(".file-panel").off().end().remove();
			}function f() {
				var b,
				    c = 0,
				    d = 0,
				    e = p.children();a.each(percentages, function (a, b) {
					d += b[0], c += b[0] * b[1];
				}), b = d ? c / d : 0, e.eq(0).text(Math.round(100 * b) + "%"), e.eq(1).css("width", Math.round(100 * b) + "%"), g();
			}function g() {
				var a,
				    b = "";"ready" === state ? b = "选中" + fileCount + "张图片，共" + c.formatSize(fileSize) + "。" : "confirm" === state ? (a = d.getStats(), a.uploadFailNum && (b = "已上传" + a.successNum + "张图片," + a.uploadFailNum + '张图片上传失败，<a class="retry" href="#">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>')) : (a = d.getStats(), b = "共" + fileCount + "张（" + c.formatSize(fileSize) + "），已上传" + a.successNum + "张", a.uploadFailNum && (b += "，失败" + a.uploadFailNum + "张")), m.html(b);
			}function h(a) {
				var b;if (a !== state) {
					switch ((n.removeClass("state-" + state), n.addClass("state-" + a), state = a, state)) {case "pedding":
							o.removeClass("element-invisible"), k.hide(), d.refresh();break;case "ready":
							o.addClass("element-invisible"), k.show(), d.refresh();break;case "uploading":
							p.show(), n.text("暂停上传");break;case "paused":
							p.hide(), n.text("确定使用");break;case "confirm":
							if ((p.hide(), n.text("确定使用").addClass("disabled"), b = d.getStats(), b.successNum && !b.uploadFailNum)) return void h("finish");break;case "finish":
							...
							} else state = "done", location.reload();}g();
				}
			}var i = this;i.modalobj.find("#li_upload a").html("上传图片"), i.modalobj.find(".modal-body").append(this.buildHtml().uploaderDialog);var j = a("#uploader"),
			    ...
			    p = l.find(".progress").hide();j.find(".btn-primary");fileCount = 0, fileSize = 0, ratio = window.devicePixelRatio || 1, thumbnailWidth = 110 * ratio, thumbnailHeight = 110 * ratio, state = "pedding", percentages = {}, supportTransition = (function () {
				...
				var d = a("#" + b.id),
				    e = d.find(".progress span");e.css("width", 100 * c + "%"), percentages[b.id][1] = c, fileid = b.id, f();
				...
				switch (a) {case "uploadFinished":
						h("confirm");break;case "startUpload":
						h("uploading");break;case "stopUpload":
						h("paused");}
		...
		}, initAudioUploader: function initAudioUploader() {
			function b(b) {
				var e = a('<li id="' + b.id + '"><p class="title" style="top:40px;">' + b.name + '</p><p class="imgWrap" style="top:30px;"></p></li>'),
					...
					switch (a) {case "exceed_size":
							text = "文件大小超出";break;case "interrupt":
							text = "上传暂停";break;default:
							text = "上传失败，请重试";}j.text(text).appendTo(e);
					...
					var c,
					    e = a(this).index();switch (e) {case 0:
							return void d.removeFile(b);case 1:
							b.rotation += 90;break;case 2:
							b.rotation -= 90;}supportTransition ? (c = "rotate(" + b.rotation + "deg)", h.css({ "-webkit-transform": c, "-mos-transform": c, "-o-transform": c, transform: c })) : h.css("filter", "progid:DXImageTransform.Microsoft.BasicImage(rotation=" + ~ ~(b.rotation / 90 % 4 + 4) % 4 + ")");
				...
				var a,
				    b = "";"ready" === state ? b = "选中" + fileCount + "个音频，共" + c.formatSize(fileSize) + "。" : "confirm" === state ? (a = d.getStats(), a.uploadFailNum && (b = "已上传" + a.successNum + "个音频," + a.uploadFailNum + '个音频上传失败，<a class="retry" href="#">重新上传</a>失败音频文件或<a class="ignore" href="#">忽略</a>')) : (a = d.getStats(), b = "共" + fileCount + "张（" + c.formatSize(fileSize) + "），已上传" + a.successNum + "个", a.uploadFailNum && (b += "，失败" + a.uploadFailNum + "个")), m.html(b);
			}function h(a) {
				var b;if (a !== state) {
					switch ((n.removeClass("state-" + state), n.addClass("state-" + a), state = a, state)) {case "pedding":
							o.removeClass("element-invisible"), k.hide(), d.refresh();break;case "ready":
							o.addClass("element-invisible"), k.show(), d.refresh();break;case "uploading":
							p.show(), n.text("暂停上传");break;case "paused":
							p.show(), n.text("继续上传");break;case "confirm":
							if ((p.hide(), n.text("确定使用").addClass("disabled"), b = d.getStats(), b.successNum && !b.uploadFailNum)) return void h("finish");break;case "finish":
							if ((n.removeClass("disabled"), b = d.getStats(), b.successNum)) {
								if (d.uploadedFiles.length > 0) return i.finish(d.uploadedFiles), void d.resetUploader();
							} else state = "done", location.reload();}g();
				}
			}var i = this;i.modalobj.find("#li_upload a").html("上传音频"), i.modalobj.find(".modal-body").append(this.buildHtml().uploaderDialog);var j = a("#uploader"),
			    k = a('<ul class="filelist"><li class="fileinput-button js-add-image" id="filePicker2" style="display:none;"> <a href="javascript:;" class="fileinput-button-icon">+</a></li></ul>').appendTo(j.find(".queueList")),
			    l = j.find(".statusBar"),
			    m = l.find(".info"),
			    n = j.find(".uploadBtn"),
			    o = j.find(".placeholder"),
			    p = l.find(".progress").hide();j.find(".btn-primary");fileCount = 0, fileSize = 0, ratio = window.devicePixelRatio || 1, state = "pedding", percentages = {}, supportTransition = (function () {
				var a = document.createElement("p").style,
				    b = "transition" in a || "WebkitTransition" in a || "MozTransition" in a || "msTransition" in a || "OTransition" in a;return a = null, b;
			})(), d;var q = { pick: { id: "#filePicker", label: "点击选择音频", multiple: !0 }, dnd: "#dndArea", paste: "#uploader", swf: "./resource/componets/webuploader/Uploader.swf", server: "./index.php?c=utility&a=file&do=upload&type=audio", chunked: !1, compress: !1, accept: { title: "Audios", extensions: "mp3,wma,wav,amr", mimeTypes: "audio/*" }, fileNumLimit: 30, fileSizeLimit: 6291456, fileSingleSizeLimit: 125829120, auto: !1 };q = a.extend({}, q, i.options.uploader), a("#dndArea p").html("最大支持 " + c.formatSize(q.fileSizeLimit) + " MB 以内的语音 (" + q.accept.extensions + " 格式)"), q.pick.multiple = i.options.multiple, d = c.create(q), d.uploadedFiles = [], d.addButton({ id: "#filePicker2", label: "+", multiple: i.options.multiple }), accept = 0, d.resetUploader = function () {
				fileCount = 0, fileSize = 0, accept = 0, d.uploadedFiles = [], a.each(d.getFiles(), function (a, b) {
					e(b);
				}), d.refresh(), d.reset(), n.removeClass("disabled"), h("pedding");
			}, d.onUploadProgress = function (b, c) {
